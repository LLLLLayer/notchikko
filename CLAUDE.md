...
Before diving into code, check whether a task-specific skill applies — each one encodes reference material this file can't fit:

- `.claude/skills/hook-guide/` — end-to-end event pipeline (IPC, Agent, Session, Approval, hook script). Consult before touching anything in `IPC/`, `Agent/`, `Session/`, `Approval/`, or the hook resources.
- `.claude/skills/theme-guide/` — SVG resolution and `NotchikkoView` rendering internals. Consult when editing `Theme/`, `NotchikkoState.swift`, `NotchikkoView.swift`, or `Resources/themes/`.
- `.claude/skills/release-guide/` — Sparkle version-bump → archive → notarize → appcast → GitHub release pipeline.
- `.claude/skills/clawd-svg-creator/` — authoring new Clawd poses / pixel animations (body templates, keyframes, palette).

`docs/hooks/{claude-code,codex,gemini-cli,trae-cli}.md` — public per-CLI hook references. Reach for these when wiring or debugging a specific agent's event schema.

`AGENTS.md` is a symlink to this file, so any edits apply to both Claude Code and Codex CLI. Keep content agent-agnostic.
...
`NotchikkoApp.swift` is the `@main` entry — a minimal SwiftUI `App` that delegates everything to `AppDelegate` via `@NSApplicationDelegateAdaptor`. `AppDelegate` is the orchestrator: it owns all modules (SocketServer, ClaudeCodeAdapter, SessionManager, NotchPanel, ApprovalManager, MenuBarManager, etc.) and wires them via callback closures. `startAgentListening()` is decomposed into focused helpers: `wireApprovalInfrastructure`, `wireSocketApproval`, `wireAdapterMetadata`, `wireDetectionFallbacks`, `handleAgentEvent`, `showNotificationCardIfNeeded`. Three peer coordinators take most of the non-trivial work: `HookOnboardingCoordinator.shared` (first-launch install prompt **and** "Setup CLI Hooks" button on the About tab — singleton, SwiftUI `HookOnboardingView` hosted in an NSPanel with three per-CLI states: `notDetected` / `notInstalled` / `installed`; `presentManually()` ignores `hasShownHookPrompt`), `ApprovalHotkeyBridge` (Cmd+Y/N ↔ ApprovalManager routing), `ApprovalPanelCoordinator` (NSPanel lifecycle + slide/fade animations). `MenuBarManager` owns the status-bar menu; its icon is a template image that flips between `MenubarDefault` / `MenubarAlert` (for `.error` / `.approving`) / `MenubarSleeping` by subscribing to `SessionManager.currentState` via `withObservationTracking` (re-subscribes after every fire). The menu bar also surfaces the Notch Detection Mode (auto / force-on / force-off) as a submenu alongside multi-display switching — rebuilt on each selection so the checkmark follows. `Views/NotchContentView.swift` is the SwiftUI root view embedded in the NotchPanel.
...
- **Agent** — `AgentBridge` protocol. `ClaudeCodeAdapter` converts `HookEvent` → `AgentEvent` via AsyncStream. Performs **synthetic session injection** — auto-yields `.sessionStart` on first event from unknown session ID. `SubagentStart`/`SubagentStop` events are silently dropped (return `nil` from `convert()`) to avoid affecting main Notchikko state. Thread-safe: `knownSessions` and `subagentDepth` are protected by `NSLock` because `onEvent` runs on a concurrent queue. AppDelegate creates a single adapter and consumes its stream directly. `AgentEvent` distinguishes three notification-ish cases: informational `.notification` (Elicitation / AskUserQuestion / plain Notification — may surface a non-blocking card), `.permissionRequest` (non-blocking PermissionRequest: hook already passed it through because approvalCard is off or bypass is on), and the direct `SocketServer.onApprovalRequest` path used for blocking PermissionRequest (carries a `request_id` and holds the hook's socket open).

- **Session** — `SessionManager` (@Observable) tracks multiple sessions, maps tools to visual states, manages idle/sleep timers (60s/120s via `resetTimers()`) plus separate return timers (5s for errors, 3s for task-complete auto-switch — `resetTimers()` cancels all three). Return/auto-switch timers go through `transition(to:)` so they respect the `isDragging`/`isApproving`/`isPetting` locks (regression risk if bypassed). `activeSessionId` prioritizes: pinned → detection tier (hook > transcript > discovered); within each tier, working phase (processing/runningTool/compacting) → most recent idle. Tiering prevents cold-start placeholders (`ProcessDiscovery`) or stale-jsonl discoveries (`TranscriptPoller`) from stealing active from the real hook session. Supports pinned session binding with auto-switch: after task completion (Stop event), celebrates 3s then auto-unpins and switches to next active session. Sessions store terminal PID, tty path, prompt text, bypass mode flag, and token usage. Max 32 concurrent sessions with LRU eviction — `evictOldestSession()` has three fallbacks (ended → idle → any working) so the dict can't grow unbounded. `onSessionRemoved` callback fires for both manual `removeSession()` and eviction, letting `ApprovalManager.cleanupSession` stay in sync. Self-heal: `transition(to: .happy)` force-clears a stuck `isApproving` flag (defensive — `.happy` implies the active session ended, so any leftover approval lock is stale).
...
- **Theme** — `ThemeProvider` resolves SVGs by state with per-state URL caching (cleared on state exit, so re-entry picks new random variant; window rebuild reuses same SVG). Built-in "clawd" theme from bundle; custom themes from `~/.notchikko/themes/{id}/` with `theme.json` manifest. External SVGs sanitized via `SVGSanitizer` (strips `<script>`, event handlers, `javascript:` URLs). If a state has no resource, `svgURL(for:)` walks `NotchikkoState.fallbackState` (e.g. `.petting → .happy`) so themes that don't ship the newer states still render something reasonable.
...
- **Petting (fun interaction)** — `DragController` also monitors `.mouseMoved` events (60Hz throttled) and accumulates path length + x-direction reversals in a 700ms sliding window. When the mouse is inside the Notchikko hit-rect and crosses the cold-start threshold (≥160pt path + 2 reversals), it fires `onPetStart`; each subsequent x-reversal fires `onPetCombo`; 900ms of no movement or leaving the hit-rect fires `onPetEnd`. `SessionManager.beginPetting` / `registerPettingCombo` / `endPetting` translate these into the `.petting` state, a per-combo `sfx-petting-{1..8}` variant (with a milestone `sfx-petting-milestone.wav` at combos 10/15/20/30/50/75/100), and a `danmakuPettingEvent` that `DanmakuView` renders as a large colored `×N` number drifting from the top. `NSHapticFeedbackManager` fires a double `.levelChange` pulse inline in the mouse-event handler (LSUIElement apps cannot haptic-feedback from a `Timer` fire). Combo throttle grows with combo count (200ms → 400ms ceiling) and resets instantly on `endPetting` — if the user pauses, they start over from 1.

- **Approval** — Separate stacked `NSPanel` windows managed by `ApprovalPanelCoordinator` that slide out from behind Notchikko (overlap 15px, level mainMenu+2 so visually behind the Notchikko panel). `ApprovalManager` (model) manages pending requests with 24h stale timer (matching hook timeout); `ApprovalPanelCoordinator` (view) owns the NSPanels + animations. All `isVisible` mutations route through `ApprovalManager.setVisibility()` which fires `onCardVisibilityChanged(requestId, visible)` — coordinator responds by animating. Approval cards only appear for `PermissionRequest` hook events (not PreToolUse — PreToolUse fires for all tools regardless of permission settings). Four buttons: Deny / Allow Once / Always Allow / Auto Approve. "Always Allow" and "Auto Approve" both send `bypass: true` in the socket response; the hook outputs `setMode: bypassPermissions` via `updatedPermissions` in a single round-trip (no flag files). `autoApprovedSessions` set for app-side fast-path; cleared via `onSessionRemoved` when a session is evicted or closed. AskUserQuestion arriving via PermissionRequest shows interactive option buttons (`onAnswer` callback sends `{answers: {question: selectedOption}}`); via PreToolUse shows non-blocking notification card (1s debounce — delays card to let PermissionRequest replace it if it arrives within ~0.5s). Cards auto-hide after 15s (configurable), hover on Notchikko restores hidden cards via `NotchHitTestView` tracking area. `onSessionEvent` only clears notification cards; three `dismissStaleApprovals` flavors auto-close blocking cards: (a) on new prompt / stop / sessionEnd for a session, (b) on a `toolUse(.post)` for the same session+tool (user authorized via the CLI's native approval, leaving a zombie card), and (c) inside `handleApproval` when a new `PermissionRequest` arrives for the same session — the hook-open fd is closed via `SocketServer.closePending(requestId:)` so the old hook unblocks before the card is dismissed. `isApproving` flag on SessionManager locks state to `.approving` until all blocking cards are dismissed (like `isDragging`). Session submenu: Pin / Jump / Close.
...
- **TranscriptPoller** (`IPC/TranscriptPoller.swift`) — Polls `~/.claude/projects/**/*.jsonl` and `~/.codex/sessions/**/*.jsonl`. Cadence: first scan at t=30s (cold-start silence — lets hook sessions establish first and lets 30s-old jsonls age out past `discoveryThreshold`), then 60s forever (t=30, 90, 150, …). Stays offset from `ProcessDiscovery`'s scans (t=60, 120, …) by 30s so the two never double-scan. Two thresholds gate what makes it through: `recentThreshold = 90s` (scan window — files older than this are never opened; slightly larger than pollInterval so adjacent scan windows overlap) and `discoveryThreshold = 75s` (new-session gate — jsonls in the 75–90s band can still be read for deltas if already known, but won't materialize a new session; also larger than pollInterval so a fresh session created right after a scan is still within the gate at the next one). Discovery also requires `ProcessDiscovery.liveAgentSources()` to show a live process of the matching source, so a transcript can't resurrect a dead session when the CLI isn't running at all. Tracks `fileOffsets` to read incrementally. Emits synthetic `AgentEvent`s into `SessionManager.handleEvent`. When a hook session arrives for the same id, `mergeWithHookSession()` drops it from its known-set so hook takes over. **Known pitfall**: Claude Code does NOT keep the jsonl file open between writes (open-write-close pattern), so `lsof` cannot be used to tell live-but-idle from truly-dead sessions — that's why the gates rely on mtime freshness instead.

- **ProcessDiscovery** (`IPC/ProcessDiscovery.swift`) — Scans `ps` every 60s for known agent process names (`claude`, `codex`, `gemini`, `traecli`, `coco`). First scan is delayed to t=60s — a cold-start silence window so background tmux instances, MCP wrappers, and orphaned CLI processes don't flood in as fake sessions the moment the app launches (`initialDelay`). Creates `discovered-{source}-{pid}` placeholder sessions; emits sessionEnd when the pid disappears. Lowest-priority detection tier — visible in menu/card so the user knows the agent is running even without hooks, but never outranks a real hook/transcript session in `activeSessionId` selection.
...
The `source` arg ("claude-code", "codex", "gemini-cli", "trae-cli") selects the JSON parser. Trae CLI emits **two payload shapes**: the newer Claude Code-style flat format (`hook_event_name` + top-level fields) and the legacy nested format (`event_type` + nested body). `handle_trae_cli` checks `hook_event_name` first and routes to `handle_trae_standard`; otherwise it falls back to the legacy branch. **All Trae events are fire-and-forget** — Trae CLI doesn't read hook stdout, so `handle_trae_standard` never generates a `request_id` or blocks, even for `PermissionRequest`. The Trae event map currently covers 13 lifecycle events (`session_start/end`, `user_prompt_submit`, `pre/post_tool_use`, `post_tool_use_failure`, `pre/post_compact`, `stop`, `subagent_start/stop`, `notification`, `permission_request`); `HookInstaller.CLIHookConfig` must register the same list or the events never reach the hook. Gemini CLI uses different event names (`BeforeAgent`→`UserPromptSubmit`, etc.) + snake_case tool names mapped via `GEMINI_TOOL_MAP`. **Only `PermissionRequest` events block** (not PreToolUse — PreToolUse fires for all tools regardless of permission settings). For PermissionRequest on approval tools (Bash/Edit/Write/NotebookEdit) or AskUserQuestion, generates UUID `request_id` and blocks waiting for response. Response format varies: approval tools get `{decision: {behavior: "allow|deny"}}`, AskUserQuestion gets `{decision: {behavior: "allow", updatedInput: {questions, answers}}}`. If app sends `bypass: true`, hook adds `updatedPermissions: [{type: "setMode", mode: "bypassPermissions", destination: "session"}]`. On Stop events, reads `transcript_path` tail to extract token usage (`input_tokens`, `output_tokens`, `cache_read`, `cache_creation`). Walks the process tree (up to 15 levels) via `detect_terminal_info()` to collect both the terminal PID and full PID chain (used by VS Code extension for terminal tab matching). **Fail-open everywhere**: any exception / missing socket / missing .py / timeout → exit 0 so the hook never blocks the CLI.

**Hook versioning**: both `notchikko-hook.sh` and `notchikko-hook.py` carry a `# notchikko-hook-version: N` stamp on line 2. `HookInstaller.isInstalledHookOutdated` reads the stamp from each installed file and compares it to the bundled copies; any mismatch / missing stamp / missing file → outdated → Settings shows a reinstall banner. Bump *both* stamps in lockstep whenever either script's wire behavior changes — otherwise users stay on the old hook until they manually reinstall.
...
`SoundManager` (`@MainActor`) plays audio on state transitions with a 2-second cooldown. `play(for:cooldownKey:)` takes an optional `cooldownKey` — `nil` dedupes per-state (the default), but callers can pass `"session-start:\(sid)"` or `"petting-\(combo)"` to widen the key so concurrent sessions / rising combos don't swallow each other. Three-tier resolution: user custom sounds (`~/Application Support/notchikko/sounds/`) → theme sounds (`themes/{id}/sounds/`, using manifest `sounds` mapping then directory scan) → built-in sounds (bundle `.wav`). Built-in lookup falls back to `sfx-<stateName>.wav` for any key not in the explicit `defaultSoundMap`, which is how `petting-1..8` and `petting-milestone` variants resolve. Soundable states: happy, error, approving, session-start, petting (+ its combo variants). Volume: continuous float 0.0–1.0 via PreferencesStore.

`prewarm()` runs from `applicationDidFinishLaunching` on a detached `userInitiated` Task. It enumerates every bundled `sfx-*.wav`, builds an `AVAudioPlayer` for each, calls `prepareToPlay()`, hands them back to the main actor via `installPrewarmed`, then plays the first one at `volume = 0` for 30ms to force CoreAudio HAL activation (some macOS versions don't actually wake HAL until `play()` is called). All players live in a `cachedPlayers: [URL: AVAudioPlayer]` dict keyed by URL — subsequent `playURL` calls do `currentTime = 0; play()` on the cached instance instead of re-instantiating, dropping the main-thread cost from ~50–200ms to near-zero. Cache misses (theme/custom URLs not covered by prewarm) lazy-load and store. **`invalidate(url:)` must be called whenever a custom sound file is replaced or removed** — `setCustomSound` and `removeCustomSound` already do this, otherwise the cached player keeps pointing at the stale buffer. Same-player consecutive plays skip the `stop()` call (only `currentTime = 0`); `stop()` releases the buffer and makes the next play *slower*.

### State Transitions

No priority-based gating — `transition(to:)` always accepts the new state (blocked by `isDragging`, `isApproving`, or `isPetting`). Note: `NotchikkoState.priority` exists but is unused — vestigial from an earlier design. Tool→State mapping: Read/Grep/Glob→reading, Edit/Write/NotebookEdit→typing, Bash→building, others→typing. Non-tool phases: processing→thinking (LLM generating), compacting→sweeping (context compaction). Error states auto-return to idle after 5s; happy (task complete) triggers 3s celebration then auto-switches session. Notification events (Elicitation/AskUserQuestion) set approving state indefinitely until next event. Approving state is interruptible by any event (like idle/sleeping). Petting enters via mouse interaction only (not event-driven); it yields to drag and approval immediately.
...
- **Adding a new CLI agent** — update three places: `CLIHookConfig` (metadata mapping), `HookInstaller` (config file registration with correct `settingsPath` **and** a `hookEvents` list that matches the event names your script branch recognizes — e.g. Trae's 13 snake_case events feed `TRAE_EVENT_MAP`; if the two lists drift, the missing events are silently dropped upstream), and the hook script (event name mapping if different from Claude Code's schema, e.g. Gemini's `GEMINI_EVENT_MAP` + `GEMINI_TOOL_MAP`). To also pick it up without a hook: add the binary name to `ProcessDiscovery.agentNames` and (if it writes JSONL transcripts) a directory to `TranscriptPoller`.
...
- **State writes must go through `transition(to:)`** — direct `currentState = X` assignments bypass the `isDragging` and `isApproving` locks, causing Notchikko to snap out of `.approving` while an approval card is still visible. `scheduleReturn` and `scheduleAutoSwitch` timer fires are the historical offenders; any future timer / callback that writes state needs to follow suit.
...
- **Bump both hook version stamps together** — `notchikko-hook.sh` and `notchikko-hook.py` each carry `# notchikko-hook-version: N` on line 2. Increment *both* stamps any time you change either script's externally-visible behavior; otherwise `isInstalledHookOutdated` won't trigger the reinstall banner and users keep running stale copies.
- **Haptic feedback must fire from the NSEvent handler, not a Timer** — `DragController`'s petting detection calls `NSHapticFeedbackManager.defaultPerformer.perform(.levelChange, ...)` directly inside the mouse-moved event handler. Scheduling it from a `Timer` fires silently in `LSUIElement` (accessory) apps; the inline double-pulse is intentional for a chunkier feel.
- **Petting combo resets on `endPetting`, not on timeout decay** — hand off to `SessionManager.endPetting()` both when the DragController detects the mouse left the hit-rect *and* when the 900ms idle timer fires. Forgetting either path leaves `pettingCombo` at its last value, and the next `registerPettingCombo` starts from the throttle ceiling instead of 200ms.
- **`sessionEnd` must not clobber an in-flight `.happy` celebration** — Trae CLI fires `stop` then `session_end` back-to-back each turn. `Stop` transitions to `.happy` and schedules a 3s `scheduleAutoSwitch`; without a guard, the very next `.sessionEnd` sees no alive sessions and immediately calls `transition(to: .sleeping)`, killing the celebration within milliseconds. `SessionManager.handleEvent` guards with `guard currentState != .happy else { break }` in the `.sessionEnd` case — any future state write in that branch must stay behind the same guard. Claude Code doesn't hit this because its `SessionEnd` only fires on real CLI exit, long after the 3s timer has already run.
